...
namespace Microsoft::VisualStudio::CppUnitTestFramework {
	template<>
	std::wstring ToString<milliseconds>(const milliseconds& t) {
		RETURN_WIDE_STRING(t.count());
	}

	template<>
	std::wstring ToString<high_resolution_clock::time_point>(const high_resolution_clock::time_point& t) {
...
namespace LibraryDesktopTests
{
	...
	public:
		...
		TEST_METHOD(UpdateGameTime)
		{
			...
			Assert::AreEqual(clock.LastTime(), gameTime.CurrentTime());
		}
